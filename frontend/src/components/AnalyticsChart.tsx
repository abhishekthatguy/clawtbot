...
function MetricBar({ label, value, maxValue, color }: MetricBarProps) {
    ...
    return (
        <div className="mb-4">
            ...
            <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: "var(--clawt-surface)" }}>
                <div
...
export default function AnalyticsChart({ title, metrics }: AnalyticsChartProps) {
    ...
    return (
        <div className="glass-card p-5 animate-fade-in">
            <h3 className="text-sm font-bold mb-4" style={{ color: "var(--clawt-text)" }}>
                {title}
            </h3>

            <MetricBar label="Likes" value={metrics.likes} maxValue={maxVal} color="#dc2626" />
            ...
            {/* Engagement Rate */}
            <div className="mt-4 pt-4 flex items-center justify-between" style={{ borderTop: "1px solid var(--clawt-border)" }}>
                <span className="text-xs" style={{ color: "var(--clawt-text-dim)" }}>Engagement Rate</span>
