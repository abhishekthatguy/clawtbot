...
const TOAST_CONFIG: Record<ToastType, { icon: string; gradient: string; border: string; glow: string }> = {
    ...
    error: {
        icon: "✕",
        gradient: "linear-gradient(135deg, rgba(220, 38, 38, 0.15), rgba(185, 28, 28, 0.08))",
        ...
        >
            <div className="flex items-start gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold" style={{ color: "var(--clawt-text)" }}>
                        {toast.title}
                    </p>
                    {toast.message && (
                        <p className="text-xs mt-0.5" style={{ color: "var(--clawt-text-dim)" }}>
                            {toast.message}
                        </p>
                    )}
                </div>
                ...
                <button
                    ...
                    style={{ color: "var(--clawt-text-dim)", background: "rgba(255,255,255,0.05)" }}
                >
                    ×
                </button>
            </div>
