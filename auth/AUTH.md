# ClawtBot Authentication Module
...
2. **SessionProvider** — A React context provider wrapping the entire app. It:
   ...
   - Listens for API-triggered expiry via `clawtbot-session-expired` custom event.
...
3. **API Client** — The `apiFetch` wrapper dispatches `clawtbot-session-expired` on 401/403 responses instead of hard-redirecting, letting the SessionProvider handle it gracefully.
...
- Matching ClawtBot dark glassmorphism theme
