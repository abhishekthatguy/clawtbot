"""
ClawtBot — Test Suite for Data Parser Agent
...
def test_parse_hashtags():
    ...
    result = _parse_hashtags("#clawtbot")
    assert result == ["#clawtbot"]
    
    result = _parse_hashtags("#clawtbot,#productdemo")
    assert "#clawtbot" in result
...
async def test_csv_parsing():
    ...
    assert "#clawtbot" in row1["default_hashtags"]
    ...
    # Check multi-hashtag row (row 4: #clawtbot,#productdemo)
    row4 = rows[3]
    assert len(row4["default_hashtags"]) == 2
    assert "#clawtbot" in row4["default_hashtags"]
...
async def test_multi_brand():
    ...
    assert "Clawtbot" in brands
    assert "Abhishek Singh" in brands
    
    clawtbot_count = sum(1 for r in rows if r["brand"] == "Clawtbot")
    abhishek_count = sum(1 for r in rows if r["brand"] == "Abhishek Singh")
    assert clawtbot_count + abhishek_count == 20
